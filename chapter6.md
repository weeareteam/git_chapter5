...
- **리모트 브랜치** 
  ...
  - 원격 저장소에서 생성한 브랜치를 리모트 브랜치라고함 
  
  - 리모트 브랜치는 보통 별칭/브랜치 이름 형태임
  
  - 원격 저장소와 연결된 로컬 저장소에서 새로운 브랜치를 생성한다 해도 원격 저장소에도 브랜치가 생성되는 것은 아니면 별도의 동기화 작업이 필요함 

- **실습 준비**

  - 깃 허브에서 새 저장소 생성
  
  - **git remote add origin 원격 저장소 주소** : git bash에서 원격 저장소 등록
  
<img width="585" alt="git remote" src="https://user-images.githubusercontent.com/114068529/195074465-2e601b3b-8ca4-4e2a-bb03-0dc95deb06f9.PNG">

- **브랜치 추적**

  - 깃의 브랜치 :  특정 커밋 해시 값을 가리키는 포인터
  
  - 브랜치 추적 : 원격 저장소의 브랜치를 가리키는 것 

- **브랜치 업로드**

  - **git remote show 명령어** : 원격 저장소의 리모트 브랜치 확인 

  - 리모트 브랜치는 서버 간 통신 후에 생성 

  - **git push 원격저장소별칭 브랜치이름** : 로컬 저장소의 브랜치를 원격 저장소에 동기화 

  - **git push -u origin master** : 선택한 브랜치를 원격 서버로 전달
  
  - 로컬 브랜치를 넣으면 원격 저장소는 로컬 저장소와 같은 브랜치 생성
  
<img width="532" alt="show origin" src="https://user-images.githubusercontent.com/114068529/195077972-e77c5cfc-5162-421f-b975-6e3590127ccb.PNG">

- **이름이 다른 브랜치**

  - 깃은 서로 다른 로컬 브랜치와 리모트 브랜치를 수동으로 지정하여 연결 가능
  
  - **git push origin 브랜치이름:새로운브랜치** : 브랜치를 수동으로 지정하여 연결

- **업스트림 트래킹**

  - 트래킹 브랜치(업스트림) : 리모트 브랜치와 로컬 브랜치를 연결해 주는 중간다리 역할

  - 새로운 업스트림을 만드는 명령어:$ git checkout --track origin/브랜치이름

- **원격 브랜치 복사**

  - 원격 저장소의 리모트 브랜치를 이용해 로컬 저장소에도 새로운 브랜치를 생성하여 동기화

- **업스트림 연결**

  - 업스트림을 직접 설정하면 원격 저장소로 트래킹 브랜치가 설정됨
   
  - **git branch -u origin/브랜치이름** : 기존 브랜치를 특정 원격 브랜치로 추적
<br>
...
- **브랜치 푸시**
  ...
  - 처음에 커밋과 브랜치를 push할 때 업스트림 설정 필요 

  - **git push --set upstream origin master** : 수동으로 트래킹 브랜치와 업스트림 설정 
<br>

- **브랜치 패치**

  - 리모트 브랜치 패치는 일반적인 커밋 패치와 동일

  - 리모트 브랜치가 패치되면 깃은 원경저장소 별칭/브랜치 포인터만 생성

  - **git merge** : 원격 저장소에 패치된 커밋을 새로운 로컬 브랜치로 반영하기 위한 병합 명령 

> **11.브랜치 삭제**
   
- 현재 자신이 있는 브랜치는 삭제 불가능 
...
- **일반적인 삭제 방법** 
  ...
  - (스테이지가 깔끔한 상태에서만 삭제를 허용!)
  
  - **git branch -d + 삭제할 브랜치 이름** : 병합되지않고, 스테이지가 깔끔한 브랜치 삭제
...
- **강제로 삭제하기**
  ...
  - (워킹 디렉토리 또는 추가 커밋 작업이 있으면 삭제가 되지않아서 이럴 경우 사용합니다)
  
  - **git branch -D  삭제할 브랜치 이름** : 대문자 D를 통해 강제 브랜치 삭제
